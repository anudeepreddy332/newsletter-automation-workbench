...
| Operator workflow correction | Explicit fetch; multi-select pickers; unified mixed-block layout; automatic MockWordPress on Generate; collapsed WordPress evidence. This is not Milestone 6. | Tests prove fetch honesty, mixed layout identity, exact ordered rendering, stale approval, and MockIterable regressions. |
